...
<?php
	...
	$csv = new CSV();
	$prelimGrade = $csv->open('../files/'.$_SESSION['faculty'].'-prelim.csv');
	$rows = 0;
		...
		foreach($prelimGrade['rows'] as $i){
			if(is_numeric($i[0]) and isStudent($i[0]) and isStudentInClass($i[0])){
				...
				if(rowExist($i[0],$_GET['class'])){
					mysql_query("update rgrades set prelim='".$i[2]."',p_absent='".$i[3]."' where student='".$i[0]."' and class='".$_GET['class']."' and ay='".enlistment('ay')."' and sem='".sem('1',enlistment('sem'))."'");
				}else{
					mysql_query("insert into rgrades(student,class,subject,ay,sem,prelim,p_absent) values('".filt($i[0])."','".filt($_GET['class'])."','".classe($_GET['class'],'subject')."','".enlistment('ay')."','".sem('1',enlistment('sem'))."','".$i[2]."','".$i[3]."')");
				}
			}
	...
	unlink('../files/'.$_SESSION['faculty'].'-prelim.csv');
